...
import com.skylerclagg.adc_hub.destinations.FavoritesViewDestination
import com.skylerclagg.adc_hub.destinations.GameManualViewDestination
import com.skylerclagg.adc_hub.destinations.LookupViewDestination
import com.skylerclagg.adc_hub.destinations.ScoreCalculatorsHomeDestination
import com.skylerclagg.adc_hub.destinations.SettingsViewDestination
import com.skylerclagg.adc_hub.destinations.TeamworkScoreCalculatorDestination
import com.skylerclagg.adc_hub.destinations.WorldSkillsViewDestination
...
class RootActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            val gameManualTab = TabBarItem(title = "Game Manual", direction = GameManualViewDestination(), selectedIcon = Icons.AutoMirrored.Filled.MenuBook, unselectedIcon = Icons.AutoMirrored.Outlined.MenuBook)
            ...
            val scoreCalculatorsTab = TabBarItem(title = "Score Calculators", direction = ScoreCalculatorsHomeDestination(), selectedIcon = Icons.Filled.Calculate, unselectedIcon = Icons.Outlined.Calculate)
            // creating a list of all the tabs
            val tabBarItems = listOf(favoritesTab, worldSkillsTab, lookupTab, gameManualTab, scoreCalculatorsTab, settingsTab)
            ...
            val routeNameMap = mapOf(
                ...
                "game_manual_view" to "Game Manual",
                "lookup_view" to "Lookup",
                "settings_view" to "Settings",
                "score_calculators_home" to "Score Calculators",
                "teamwork_score_calculator" to "Teamwork Score Calculator",
                "autonomous_flight_skills_calculator" to "Autonomous Flight Skills Calculator",
                "piloting_skills_calculator" to "Piloting Skills Calculator"
            )
                ...
                Surface(
                    ...
                    Scaffold(
                        ...
                        DestinationsNavHost(
                            ...
                            composable(GameManualViewDestination) {
                                GameManualView(
                                    navController = navController
                                )
                            }
                            composable(ScoreCalculatorsHomeDestination) {
                                ScoreCalculatorsHome(navController = navController) // Pass the navController if needed
                            }
                            composable(TeamworkScoreCalculatorDestination) {
                                TeamworkScoreCalculator()
                            }
                            composable(AutonomousFlightSkillsCalculatorDestination) {
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun RootActivity(navController: NavController) {
    DestinationsNavHost(
        navController = navController as NavHostController,
        navGraph = NavGraphs.root,
        startRoute = FavoritesViewDestination
    ) {
        // Adding the missing route for "teamwork_calculator"
        composable(TeamworkScoreCalculatorDestination) { TeamworkScoreCalculator() }
